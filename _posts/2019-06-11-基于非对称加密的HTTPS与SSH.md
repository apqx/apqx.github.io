...
<img class="materialboxed responsive-img" src="https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/https.png" alt="pic">
...
<img class="materialboxed responsive-img" src="https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/httpsCA.png" alt="pic">
...
<img class="materialboxed responsive-img" src="https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/ssh.png" alt="pic">
...
`known_hosts`上文已经解释过了，`id_rsa`和`id_rsa.pub`是本机生成的一组`非对称加密`的`私钥`和`公钥`，`authorized_key`是本机在作为服务器的时候，远程的客户端可以把它自己的`公钥`提前保存在这个文件里，当客户端要登陆时
...
<img class="materialboxed responsive-img" src="https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/sshKey.png" alt="pic">
...
会发现，已经不会再要求输入密码了，即`VPS`服务器已经成功添加了客户端的`公钥`，这个`公钥`也会被称为`SSH KEY`，在`GitHub`的`Personal settings`->`SSH and GPG keys`中，就可以添加自己的`SSH KEY`，如此一来，就可以很方便的直接远程控制`git`仓库。

<img class="materialboxed responsive-img" src="https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/gitSSH.jpg" alt="pic">
