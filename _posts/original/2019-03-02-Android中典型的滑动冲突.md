...
author: 立泉
mention: NestedComponents View MotionEvent Dispatch Intercept
date: 2019-03-02 19:30:00 +0800
description: Android 触控事件在 View 树中自上而下分发传递，每一层都可以拦截、处理事件并把结果上报。“滑动”操作的本质是 View 接收触控事件，根据触控坐标变化移动内容位置。
image: 
tags: Code Android View MotionEvent Scroll
featured: true
---

Android 触控事件在`View`树中自上而下分发传递，每一层都可以拦截、处理事件并把结果上报。“滑动”操作的本质是`View`接收触控事件，根据触控坐标变化移动内容位置。
