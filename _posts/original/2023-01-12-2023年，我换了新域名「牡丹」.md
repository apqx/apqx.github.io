...
对于非开发者，需要补充一些关于域名的简单解释。服务器是现代互联网的基础设施，网站搭建在服务器上，在网络世界里只能通过 IP 地址去定位到一台特定服务器。使用 HTTP 协议可在浏览器中输入一串 http://185.199.109.153:80 字符来访问搭建在 IP 地址为 185.199.109.153 的服务器上监听 80 端口的网站，全是数字很难记对吧，所以域名应运而生。

域名可以由字母、数字甚至汉字组成，有意义的词自然比无规则数字清晰简洁，比如哔哩哔哩的拼音域名 bilibili.com，在浏览器中输入就能跳转到哔哩哔哩网站。这个过程中浏览器做了 2 件事，先去 DNS 域名服务器查询域名对应的 IP 地址，再去访问 IP 对应的服务器，中间有很多复杂细节需要更多协议处理，但总之域名起到的就是这个作用。
...
这个博客托管在 GitHub Pages 上，通过仓库根目录中的 CNAME 文件标记网站域名，且仅允许标记一个。为 Pages 配置过自定义域名应该知道，交给它托管的网站 DNS 全部指向同一组入口服务器，Pages 会在这里检测目标域名与记录的已知域名对比从而定位到要访问的网站。这种机制下如果 DNS 添加一条 CNAME 记录把 apqx.me 指向 mudan.me，即指向 GitHub Pages 的入口服务器，其检测到目标域名 apqx.me 在所有仓库的 CNAME 中并不存在会直接返回 404 页面，所以这种方式在这种情况下是不可行的。
...
车到山前自有路，我一直在使用 Cloudflare 的 DNS 服务，众所周知 Cloudflare 本职是 CDN 内容分发网络，只需把 DNS 解析的 Proxy status 设置为 Proxied 即可启用它的 CDN。访问 apqx.me 的流量会先经过 Cloudflare 再到达托管网站的 GitHub Pages，此时 Cloudflare 作为中间人是能够对经过的 HTTP 请求返回 301 重定向的。

*请忽略我在 A 记录里随意填写的 8.8.8.8，配置重定向后 HTTP 请求永远不会到达这个 IP，可任意填。*
