...
这个博客托管在 GitHub Pages 上，通过仓库根目录中的 CNAME 文件标记网站域名，且仅允许标记一个。为 Pages 配置过自定义域名应该知道，交给它托管的网站 DNS 全部指向同一个入口服务器，Pages 会在这里检测目标域名与记录的已知域名对比从而定位到要访问的网站。这种机制下如果 DNS 添加一条 CNAME 记录把 apqx.me 指向 mudan.me，即指向 GitHub Pages 的入口服务器，其检测到目标域名 apqx.me 在所有仓库的 CNAME 中并不存在会直接返回 404 页面，所以这种方式在这种情况下是不可行的。
