...
`SSH`除了支持普通的`账号+密码`登录方式外，还支持直接使用用户`私钥`进行无密码登录，或者说，经用户`私钥`加密的特定数据就是身份凭证。
...
`known_hosts`上文已经解释过了，`id_rsa`和`id_rsa.pub`是本机生成的一组`非对称加密`的`私钥`和`公钥`，`authorized_keys`是本机在作为服务器的时候，远程的客户端可以把它自己的`公钥`提前保存在这个文件里，当客户端要登录时：
...
此时在本地用户的`.ssh`目录下，会出现私钥`id_rsa`和公钥`id_rsa.pub`2个文件，我有一台`VPS`服务器，先使用账号密码登录：
...
此外，在`GitHub`的`Personal settings`->`SSH and GPG keys`中也可以添加自己的`SSH Key`，如此一来，就可以很方便的直接远程控制`git`仓库，而无需密码。
