...
author: 立泉
mention: 公钥与私钥 证书 签名 网络攻击 MITM
date: 2019-06-11 19:30:00 +0800
...
性能方面，非对称加密比对称加密复杂得多，所以对算力和资源要求更高。
...
为建立安全可靠的通信，HTTPS (HTTP Secure) 应运而生，对 HTTP 缺点全面改进，相应特点是：
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20190611/https_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="HTTPS and HTTP" }

HTTP 和 HTTPS 本质向下发送的都是明文，只不过 HTTP 数据包直接被 TCP/IP 传输，HTTPS 则是将数据包发给 SSL 加密后再通过 TCP/IP 传输，其中基于证书的身份验证由 TLS 完成。
...
签名生成方式：使用散列函数（如 SHA256）计算该明文信息的 Message Digest 数字摘要，再用 CA 私钥对摘要加密，密文即签名。
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20190611/https_ca_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="HTTPS establish" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20190611/ssh_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="SSH establish" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20190611/ssh_key_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="SSH authorization" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20190611/ssh_github_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="Github clone with SSH" }