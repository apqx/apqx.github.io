...
如果在中文社区中搜索`暗黑模式`，会发现很多人并没有理解`DarkMode`和`Theme`之间的关系，所以我依旧认为阅读[官方文档](https://developer.android.com/guide/topics/ui/look-and-feel/darktheme){: target="_blank" }是学习`Android`最基本也是最有效的方式。`Android`团队会把新特性的关键点和相关内容写在文档中，希望把自己的工作成果详细且准确的传递给开发者，包括这些功能的设计思路、适配方式和向前的兼容方案。很多微妙的细节确实会在社区的观点传递中丢失，所以如果只能看到质量很差的末端“教程”，知其然而不知其所以然，必定会一头雾水。
