...
做出来的`UI`还是很不错的，完美搭配`Material Design`，不过调用搜索`API`并不免费，[费用每千次5美元](https://developers.google.com/custom-search/docs/overview){: target="_blank"}。
...
实现的过程也无非是用`JavaScript`把`HTTP`请求到的数据填充到页面上显示出来而已，唯一要注意的是请求搜索的参数中有一个用作身份验证的`API KEY`，它来自于`Google Cloud Platform`的某个指定`Project`（搜索服务的账单就是与它绑定的），默认情况下这个`KEY`的权限是不被限制的，用它就可以控制整个`Project`，所以非常危险。实际上，当我把这个`KEY`写入到`JS`文件并`PUSH`到`GitHub Pages`上时，就立刻收到了`Google`发来的安全警告邮件。
