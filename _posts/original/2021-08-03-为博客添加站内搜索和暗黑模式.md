...
description: 搜索无非2种方案，使用搜索引擎提供的站内搜索服务，或完全自己实现搜索。后者更加灵活且不依赖搜索引擎缓慢的索引更新，优势很大，只是暂时不适合我，有太多的问题需要解决。所以最终选定Google的站内搜索，一来是因为我的博客早就加入到了Google的索引中，可以直接使用，二来也是一个接触当前“最好搜索引擎”所提供的开发者服务的一个机会。
cover: 
...
我并非`Web`开发者，对`HTML`、`CSS`、`JS`、`NPM`仅仅是浅尝可用，花了些时间，解决了些问题，终于基于`Google`的[Material Design Components](https://github.com/material-components/material-components-web){: target="_blank" }重构了这个[博客小站](https://github.com/apqx/apqx.github.io){: target="_blank" }。继承之前的页面设计，削减阴影，增加色彩，简洁紧凑而不失淡雅，虽还未达到设想中的完美状态，但以我当前的审美来看，十分还是可以给八分的。

# 站内搜索

[Jekyll](https://jekyllrb.com){: target="_blank" }原生支持分类管理文章的`tag`，在布局文件中可以轻易的用[Liquid](https://jekyllrb.com/docs/posts/#tags){: target="_blank" }语法获取每一个`tag`和标记了该`tag`的文章列表，比如这篇就标记了`CS`、`Google`和`DarkMode`3个`tag`，点击文章顶部的`标签`能看到所有相关联的文章弹窗。但是`tag`毕竟有限，不可能把文章里的每个关键词都设置为`tag`，所以站内搜索自然就是一个非常诱人的功能点，随着写过的东西越来越多，我有时候也很想知道自己在多少文章中提到过某些相似的内容。

搜索无非2种方案，使用搜索引擎提供的站内搜索服务，或完全自己实现搜索。后者更加灵活且不依赖搜索引擎缓慢的索引更新，优势很大，只是暂时不适合我，有太多的问题需要解决。所以最终选定`Google`的站内搜索，一来是因为我的博客早就加入到了`Google`的索引中，可以直接使用，二来也是一个接触当前“最好搜索引擎”所提供的开发者服务的一个机会。
...
`Google`也提供面向站内搜索的[Programmable Search](https://cse.google.com/cse/all){: target="_blank" }服务，它实际上是允许用户创建一个自定义搜索范围的`Google`搜索，甚至可以自定义搜索显示的部分`UI`，也支持把`Google`的搜索框和搜索结果嵌入到用户自己的网页中。我试着嵌入，但发现`UI`风格和我的博客完全不搭，而且还存在一些搜索结果不定引起的元素尺寸变化问题，所以我虽然不擅长`Web`编程，踌躇后还是决定手动调用[Google的搜索API](https://developers.google.com/custom-search/v1/introduction){: target="_blank" }而不是使用现成的`UI`组件。
...
做出来的`UI`还是很不错的，完美搭配`Material Design`，不过调用搜索`API`是有费用的，[每千次5美元](https://developers.google.com/custom-search/docs/overview){: target="_blank"}，对于访问量本来就很小的站点来说其实也差不多就是免费了。
...
实现过程也无非是用`JavaScript`把`HTTP`请求到的数据填充到页面上显示出来而已，唯一要注意的是请求搜索的参数中有一个用作身份验证的`API KEY`，它来自于`Google Cloud Platform`的一个启用了`Custom Search API`的`Project`（搜索服务的账单就是与它绑定的），默认情况下`Project`的`KEY`权限很大，用它可以控制整个`Project`的行为，所以非常危险。实际上，当我把这个`KEY`写入到`JS`文件并`PUSH`到`GitHub Pages`上时，就立刻收到了`Google`发来的安全警告邮件。
...
是的，基本实现就是这么简单，只是要与`SASS`和`Material Design Components`组合，我的确遇到了相当多的问题，有的甚至困扰几天都百思不解，过程很曲折，但好在都能被一一解决，所以看到最终的成品，我也是十分欣慰了。
