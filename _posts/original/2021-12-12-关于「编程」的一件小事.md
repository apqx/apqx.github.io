...
又过了几天，突然回过神来，只是下载图片而已，为什么一定要用`Downie`呢？当时之所以这么做是因为遇到了一个问题，同一个`URL`，用`Downie`可以下载到原图，而直接用浏览器打开或用`Postman`请求拿到的却是毫无意义的占位图。那会儿还有别的事情要做就没有深究，既然`Downie`能用而且还很好用，何乐不为呢。
...
然后重写了程序的下载逻辑，不再抛给`Downie`而是自己实现了一个下载器，把解析到的图片`URL`抛到线程池中执行，并发量设置为5的话速度还不错，而且摆脱了`macOS`的特有程序`Downie`意味着这个下载器可以在任何平台的`JVM`上运行，真正的跨平台。
