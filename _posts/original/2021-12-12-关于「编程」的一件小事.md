...
author: 立泉
actor: 单雯
mention: 剧照下载 新浪博客 链接错乱 防盗链
date: 2021-12-12 19:30:00 +0800
description: 作为软件工程师，我知道编程可以做什么，但不讳言，工作之外除去鼓捣过一些软硬结合的玩具我从没有用它解决过日常遇到的问题。直到昨天，虽然是一件小事但值得记录，第一次觉得程序像魔法一样“神奇”，心态开始有“欣慰”变化。
image: https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20211212/idea_run_download_980_thumb.jpg
tags: Code Blog 新浪博客 昆曲 单雯 下载 Referer
---

作为软件工程师，我知道编程可以做什么，但不讳言，工作之外除去鼓捣过一些软硬结合的玩具我从没有用它解决过日常遇到的问题。直到昨天，虽然是一件小事但值得记录，第一次觉得程序像魔法一样“神奇”，心态开始有“欣慰”变化。

几个月前，我在新浪博客上找到很多剧照，这些照片质量很高，保留着相机拍摄时的 Metadata 元信息，我想把它们下载收藏起来。其中大部分只要点击网页里的缩略图即可跳转到原图然后下载，但有些照片跳转却会一直加载直到超时也不显示。那时我以为是新浪图床已经停止服务，可能原图已经不存在，就只保存模糊的缩略图。
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20211212/downloaded_pic_thumb.webp){: loading="lazy" class="clickable clickShowOriginalImg" alt="Downloaded pictures" }

至此，我基本猜到为什么有的博文能跳原图而有的不能，一定是开发者在某次迭代中错误的给某些原图 URL 添加上那个前缀但没有搭配正确的解析 JS，加上如今博客凋零，人微言轻，可能他们到现在都没有发现这个问题。我甚至怀疑，以目前新浪博客的状态真的有人在维护吗🤔。

Now，已经知道之前不能下载的原图都还默默存储在新浪服务器上，当时保存的缩略图文件名即是上面提到的图片 ID。不过多达数百张，要如何下载呢？

手动一张张拼接 URL 显然不现实，需要写一个“程序”替我完成读取缩略图文件名、拼接 URL 再批量发送到 Downie 下载队列这三个连续操作，即是下面这段代码所做的事：
...
注意到代码分为两步，下载后有一个“合并”操作，是因为向 Downie 添加下载任务的命令参数有限，只能指定 URL 和下载目录而不能指定存储文件名。这些照片的默认文件名是相同的`default_s_bmiddle.gif`，下载到同一个目录会相互覆盖。
...
所以我把每一张照片都放到用它 ID 命名的文件夹中，全部下载完成后按一下 Enter 键执行“合并”，最终获得数百张以 ID 命名的剧照。

![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20211212/idea_run_thumb.webp){: loading="lazy" class="clickable clickShowOriginalImg" alt="IntelliJ IDEA executor" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20211212/opera_pics_thumb.webp){: loading="lazy" class="clickable clickShowOriginalImg" alt="Downloaded pictures in finder" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/original/20211212/idea_run_download_980_thumb.jpg){: loading="lazy" class="clickable clickShowOriginalImg" alt="IntelliJ IDEA execute" }

## 再进化

又过几天，回过神来，下载图片为什么一定要用 Downie 呢？当时这么做是因为遇到一个问题，同一个 URL 用 Downie 可以下载到原图，直接用浏览器打开或用 Postman 请求拿到的却是无意义的占位图。那时有别的事情要做没有深究，既然 Downie 能用而且很好用，何乐不为。
...
可是当我用 Postman 执行请求时虽然能看到相同的 2 次重定向，URL 也一模一样，但最终拿到的却总是很小的占位图，问题出在哪里呢？

梳理几遍请求过程注意到一个关键不同点，Referer。它在 HTTP 请求的 Header 里携带请求发起者的 Hostname，通常“防盗链”机制就是服务器去检查这个参数标识的请求来源是否是自身站点来避免其它网站盗用资源。为什么注意到它呢，因为我这个博客托管在阿里云 OSS 里的图片就在使用同样的防盗链，不久前配置过域名白名单。
