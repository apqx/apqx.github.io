...
description: 成功下载到了我苦寻许久的雯明版《牡丹亭》。
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/screen_iphone_opera_mudanting.jpg){: loading="lazy" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/screen_mac_wifi_share.png){: loading="lazy" }

当手机开始播放视频的时候，观察`Wireshark`捕捉到了大量指向同一个`IP`的`TCP`数据包。

![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/screen_wireshark_opera.jpg){: loading="lazy" }

试着`Follow`它的`TCP Stream`，会发现它们所属的`HTTP`请求。

![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/screen_wireshark_flow.jpg){: loading="lazy" }
...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/pic/screen_safari_opera_mudanting.jpg){: loading="lazy" }

惊鸿一瞥，单雯和施夏明的[「山桃红」后半段（YouTube）](https://www.youtube.com/watch?v=_LngsAMjR0g){: target="_blank" }，我真的是第一次看到。
