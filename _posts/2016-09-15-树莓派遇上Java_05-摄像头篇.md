...
![](https://apqx.oss-cn-hangzhou.aliyuncs.com/blog/20160915/pi_cam.jpg){: loading="lazy" }

`Pi Cam`接在`树莓派`中部的`CSI`接口上，不需要额外供电，这里再次强烈建议为`树莓派`提供独立电源通过`USB`口供电而不要使用`GPIO`针脚供电，否则在摄像头开启的瞬间`树莓派`可能因为电压不稳而重启。

在我的项目中，因为只是玩具，使用的是一种很简单的传输图像的方法，基本原理是，由`MJPG-Streamer`调用摄像头拍摄画面并将画面发送到指定的`端口`上，在同一局域网中，浏览器访问`树莓派``IP`的该`端口`即可显示实时画面。
